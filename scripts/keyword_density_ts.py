...
wobj = wapf.wapf(baseUrl, browser) #init baseUrl and browser
...
for link in f3:
    ...
    if not link.strip():
	    ...
	    if(status == '200'):
		    ...
		    print 'Link: ' + link
		    print '--------------------------------------------------------------------------------------------------------------------------------------'
		    print templateTwo.format('Page Title:', pageTitle)
		    print templateTwo.format('Total Pages Indexed:', str(count))
		    print templateTwo.format('Total Links on Page:', linksCount)
		    print templateTwo.format('Total Word Count:', str(totalWords))
		    print templateTwo.format('Total Bytes Transferred:', total_transfer_size + ' bytes')
		    print templateTwo.format('Page Request Sent:', wobj.httpRequestSend(link))
		    print templateTwo.format('Page Reponse Received:', wobj.httpReponseReceived(link))
		    print templateTwo.format('Page Content Transferred:', wobj.httpContentTransferred(link))
		    print '--------------------------------------------------------------------------------------------------------------------------------------'
		    
		    #TODO: Write stuff below to csv file

		    print template.format("KEYWORD", "COUNT", "PERCENTAGE") # header		    
		    ...
		    for word, freq in sorted(result.iteritems(), key=operator.itemgetter(1), reverse=True):
	    		...
	    		if(freq > 1):	    					    	
		    		print template.format(word, str(freq), str(round(percentage, 4)) + '%')
		    	else:
		    		print template.format(word, str(freq), str(round(percentage, 4)) + '%')
